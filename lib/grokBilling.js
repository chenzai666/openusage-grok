...
// Must track current Grok CLI (old 0.2.x headers get a stripped unified
// payload: no creditUsagePercent / productUsage → cards stay 「警告」).
...
async function fetchOneAccount(entryKey, entry, { forceRefresh = false, runChat = false } = {}) {
  ...
  // Always surface tier in planLine (reference: "tier 4 · X Premium+")
  const planPart = card.planName || freshEntry.plan_name || "Grok";
  if (tierBadge) {
    card.planLine = tierBadge.text + " · " + planPart;
  } else if (tier != null && tier !== "") {
    card.planLine = "tier " + String(tier) + " · " + planPart;
  } else {
    card.planLine = planPart;
  }
  ...
  const hasPeriod = !!(weeklyEnd || (period && period.start));
  const isUnified = !!(creditsConfig && creditsConfig.isUnifiedBillingUser === true);
  const planHint = String(card.planName || freshEntry.plan_name || "").toLowerCase();
  const isHeavy = /\bheavy\b/.test(planHint);

  let usagePercentRaw = creditsConfig ? Number(creditsConfig.creditUsagePercent) : NaN;
  let weeklyInferred = false;
  if (!Number.isFinite(usagePercentRaw) && creditsConfig) {
    const odUsed = unitsValue(creditsConfig.onDemandUsed);
    const odCap = unitsValue(creditsConfig.onDemandCap);
    if (odUsed != null && odCap != null && odCap > 0) {
      usagePercentRaw = (odUsed / odCap) * 100;
    } else if (hasPeriod && !isHeavy) {
      // proto3 omit = 0%; old CLI headers also strip this field
      usagePercentRaw = 0;
      weeklyInferred = true;
    }
  }
  ...
  if (isUnified) {
    if (hasWeeklyPercent && !weeklyInferred) {
      card.unifiedNote = "统一账单账号：周限百分比已返回";
    } else if (weeklyInferred) {
      card.unifiedNote = "统一账单：接口未返回百分比字段，周期内按 0% 显示";
    } else {
      card.unifiedNote = "统一账单账号：周限百分比未返回；已显示 Build/API/月额度（若接口有）";
    }
  }
  ...
  const capPart =
    onDemandCapUnits != null && onDemandCapUnits > 0
      ...
      : "--";
  // 参考文案：已用 -- · US$0.00 / -- · 重置 MM/DD HH:mm
  card.onDemandText =
    "已用 -- · US$" +
    usedUsd +
    " / " +
  ...
  // 参考：解析 4 条: 周限✓ · Build✗ · API✓ · 原始数据 %APPDATA%\OpenUsage\debug\
  const flags = [
    "周限" + (hasWeeklyPercent ? "✓" : "✗"),
  ...
  card.parseSummary =
    "解析 4 条: " +
    flags.join(" · ") +
    " · 原始数据 %APPDATA%\\OpenUsageGrok\\debug\\";
  ...
  } else if (isUnified && !hasWeeklyPercent && card.apiPercent == null && !hasPeriod) {
    card.status = "警告";
    card.statusColor = "#f59e0b";
  } else {
    card.status = "正常";
