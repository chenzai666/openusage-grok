...
async function pollDeviceLogin() {
  ...
  if (resp.ok && data.access_token) {
    ...
    if (!entryKey) {
      // Reuse same vault slot if this email already exists (avoid duplicate cards)
      entryKey = findExistingKeyByEmail(email) || makeEntryKey(clientId, payload, email);
    }
    // Never use short key without identity — that overwrites every login into one slot
    if (entryKey === `https://auth.x.ai::${clientId}`) {
      entryKey = makeEntryKey(clientId, payload, email);
    }
    const prev = (loadAccounts({ recoverFromCli: false }).accounts || {})[entryKey] || {};
    const expiresIn = Number(data.expires_in) || 3600;
    const entry = {
      ...prev,
      key: data.access_token,
      ...
      labels: prev.labels || [],
      auth_mode: "oidc",
      source: prev.source || "device-login",
