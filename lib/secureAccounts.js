 ...
 * Prefer: existing vault slot by sub → by email → long key with sub → email-tagged → raw CLI key.
 * Matching existing slots is critical after local `grok login` so tokens refresh in-place.
 */
function resolveCliEntryKey(cliKey, entry, vaultAccounts) {
  ...
  // 1) Same user_id / JWT sub already in vault (device-login or prior CLI import)
  if (sub) {
    const bySub = Object.entries(accounts).find(([k, e]) => {
      if (!e || typeof e !== "object") return false;
      const esub = String(e.user_id || e.sub || e.principal_id || "").trim();
      if (esub && esub === sub) return true;
      const parts = String(k).split("::");
      if (parts.length >= 3 && parts[parts.length - 1] === sub) return true;
      return false;
    });
    if (bySub) return bySub[0];
  }

  // 2) Same email already in vault
  if (emailNorm) {
    const existingByEmail = Object.entries(accounts).find(
      ([, e]) =>
        e &&
  ...
  // 3) Expand short CLI scope keys to identity-bearing keys
  if (sub) return `https://auth.x.ai::${clientId}::${sub}`;
  if (emailNorm) return `https://auth.x.ai::${clientId}::email:${emailNorm}`;
  return cliKey;
}

/** Pull credential fields from a CLI auth.json entry (always prefer CLI after re-login). */
function credentialsFromCliEntry(entry, token) {
  const refresh =
    (typeof entry.refresh_token === "string" && entry.refresh_token.trim()) ||
    (typeof entry.refresh === "string" && entry.refresh.trim()) ||
    "";
  const patch = {
    key: token,
    auth_mode: entry.auth_mode || "oidc",
  };
  if (refresh) patch.refresh_token = refresh;
  if (typeof entry.expires_at === "string" && entry.expires_at) {
    patch.expires_at = entry.expires_at;
  }
  if (typeof entry.id_token === "string" && entry.id_token) {
    patch.id_token = entry.id_token;
  }
  if (entry.oidc_client_id) patch.oidc_client_id = entry.oidc_client_id;
  if (entry.oidc_issuer) patch.oidc_issuer = entry.oidc_issuer;
  if (entry.user_id || entry.sub || entry.principal_id) {
    patch.user_id = entry.user_id || entry.sub || entry.principal_id;
  }
  if (entry.email || entry.user_email) {
    patch.email = entry.email || entry.user_email;
  }
  if (entry.tier != null) patch.tier = entry.tier;
  return patch;
}

/** Epoch ms for token expiry (expires_at or JWT exp). */
function credentialExpiryMs(entry) {
  if (!entry || typeof entry !== "object") return NaN;
  if (typeof entry.expires_at === "string" && entry.expires_at) {
    const t = Date.parse(entry.expires_at);
    if (Number.isFinite(t)) return t;
  }
  const tok = (entry.key || entry.access_token || "").trim();
  if (tok) {
    try {
...
/**
 * True when vault credentials are clearly newer than CLI (e.g. just browser device-login).
 * Background CLI soft-sync must not stomp fresher device-login tokens for the same account.
 */
function vaultCredentialsAreFresher(vaultEntry, cliEntry, cliToken) {
  const vaultExp = credentialExpiryMs(vaultEntry);
  const cliExp = credentialExpiryMs({ ...cliEntry, key: cliToken });
  if (!Number.isFinite(vaultExp)) return false;
  if (!Number.isFinite(cliExp)) return true;
  // 2s skew: only keep vault if meaningfully newer
  return vaultExp > cliExp + 2000;
}
...
/**
 * Drop tombstones that match a CLI entry identity (key / sub / email).
 * Used by explicit "同步 Grok CLI" so users can re-import without browser re-login.
 */
function liftCliIdentityTombs(deletedKeys, cliKey, entry, entryKey) {
  const { sub, emailNorm } = readCliIdentity(entry, cliKey);
  const drop = new Set([entryKey, cliKey].filter(Boolean));
  if (sub) drop.add(`sub:${sub}`);
  if (emailNorm) drop.add(`email:${emailNorm}`);
  // Also drop preferred long key even if resolve used an existing vault slot
  const { clientId } = readCliIdentity(entry, cliKey);
  if (sub) drop.add(`https://auth.x.ai::${clientId}::${sub}`);
  if (emailNorm) drop.add(`https://auth.x.ai::${clientId}::email:${emailNorm}`);
  const before = (deletedKeys || []).length;
  const next = (deletedKeys || []).filter((k) => !drop.has(k));
  return { keys: next, lifted: before - next.length };
}

/**
 * Sync CLI auth.json into vault:
 * - add/update CLI keys
 * - auto/background: skip identities tombstoned in deletedKeys (deleted stay deleted)
 * - force (manual button): clear tombs for current CLI identities and re-import
 * - always expand short CLI scope keys to user_id/email long form
 * - never remove vault-only (browser-login) accounts
 *
 * @param {boolean} persist
 * @param {{ force?: boolean }} [opts] force=true for explicit user "同步 Grok CLI"
 */
function syncFromCli(persist = true, opts = {}) {
  ...
  // Explicit sync: lift tombs for whoever is currently in CLI auth.json so
  // "同步 Grok CLI" re-imports without forcing browser device-login again.
  if (force) {
    for (const [key, entry] of Object.entries(cli)) {
      ...
      const wasTombstoned = isCliEntryTombstoned(
        new Set(data.deletedKeys || []),
        entryKey,
        key,
        entry
      );
      const lifted = liftCliIdentityTombs(data.deletedKeys, key, entry, entryKey);
      data.deletedKeys = lifted.keys;
      result.liftedTombs += lifted.lifted;
      if (wasTombstoned && lifted.lifted > 0) result.restored++;
      if (lifted.lifted > 0) dirty = true;
    }
  }
  ...
  for (const [key, entry] of Object.entries(cli)) {
    ...
    // Migrate vault row off the shared short CLI scope key onto identity key first,
    // so we can detect "already in vault" and always refresh tokens after grok login.
    ...
    // Tombstone only blocks *re-adding* a deleted identity.
    // If the account is still in the vault, always pull latest tokens from CLI
    // (local `grok login` / token refresh must not require browser re-login).
    if (!prev && !force && isCliEntryTombstoned(deleted, entryKey, key, entry)) {
      result.tombstoned++;
      result.skipped++;
      continue;
    }

    const creds = credentialsFromCliEntry(entry, token);
    result.cliCount++;
    if (!prev) {
      data.accounts[entryKey] = {
        ...
        source: "cli-import",
        email: creds.email || email || entry.email,
        user_id: creds.user_id || sub || entry.user_id,
      };
      result.added++;
      dirty = true;
    } else {
      // Explicit 「同步 Grok CLI」 (force) always takes CLI tokens.
      // Background soft-sync must NOT overwrite a fresher browser device-login
      // for the same account — that made "same account as CLI" web re-login
      // look broken until the user clicked CLI sync again.
      const keepVaultCreds = !force && vaultCredentialsAreFresher(prev, entry, token);
      if (keepVaultCreds) {
        data.accounts[entryKey] = {
          ...prev,
          // refresh non-secret profile bits only
        ...
        result.keptFresher = (result.keptFresher || 0) + 1;
        // not counting as token update
      } else {
        data.accounts[entryKey] = {
          ...prev,
          ...entry,
          ...
          source: force ? "cli-import" : prev.source || "cli-import",
          email: creds.email || email || prev.email || entry.email,
          user_id: creds.user_id || sub || prev.user_id || entry.user_id,
          tier: entry.tier != null ? entry.tier : prev.tier,
        };
        result.updated++;
        dirty = true;
  ...
  if (!force && result.tombstoned > 0 && result.added === 0 && result.updated === 0) {
    result.message =
      `CLI 中有 ${result.tombstoned} 个账号曾在本应用删除，后台同步已跳过。` +
      `点「同步 Grok CLI」可强制重新导入，无需浏览器再登录。`;
  } else if (force && result.restored > 0) {
    result.message = `已重新导入 ${result.restored} 个此前删除的 CLI 账号`;
  }
...
/**
 * Soft import from CLI.
 * @param {boolean} persist
 * @param {{ force?: boolean }} [opts] pass force:true for explicit UI sync button
 */
function softImportCli(persist = true, opts = {}) {
  return syncFromCli(persist, opts);
}
...
/**
 * Import already-normalized credential objects (Cliproxy / pasted JSON).
 * Same identity matching as CLI sync. force=true re-adds previously deleted accounts.
 */
function importCredentialEntries(list, opts = {}) {
  const force = opts.force !== false;
  const source = opts.source || "cliproxy-import";
  const result = {
    added: 0,
    updated: 0,
    skipped: 0,
  ...
  if (!Array.isArray(list) || list.length === 0) {
    result.error = result.error || "没有可导入的账号";
    return result;
  }
  ...
  for (const raw of list) {
    ...
    const token = String(raw.key || raw.access_token || "").trim();
    if (!token) {
      result.skipped++;
      continue;
    }
    const clientId = raw.oidc_client_id || DEFAULT_OIDC_CLIENT_ID;
    const cliKey = `https://auth.x.ai::${clientId}`;
    const { email, sub } = readCliIdentity(raw, cliKey);
    if (email) result.emails.push(email);
    const entryKey = resolveCliEntryKey(cliKey, raw, data.accounts);
    const creds = credentialsFromCliEntry(raw, token);
    let prev = data.accounts[entryKey];

    if (force) {
      const lifted = liftCliIdentityTombs(data.deletedKeys, cliKey, raw, entryKey);
      data.deletedKeys = lifted.keys;
      result.liftedTombs += lifted.lifted;
      if (!prev && lifted.lifted > 0) result.restored++;
    } else if (!prev && isCliEntryTombstoned(new Set(data.deletedKeys || []), entryKey, cliKey, raw)) {
      result.skipped++;
      continue;
    }
    ...
    if (!prev) {
      data.accounts[entryKey] = {
        ...
        source,
        email: creds.email || email || raw.email,
        user_id: creds.user_id || sub || raw.user_id,
      };
      result.added++;
      dirty = true;
    } else {
      data.accounts[entryKey] = {
        ...prev,
        ...creds,
        labels: Array.isArray(prev.labels) ? prev.labels : raw.labels || [],
        source,
        email: creds.email || email || prev.email || raw.email,
        user_id: creds.user_id || sub || prev.user_id || raw.user_id,
        tier: raw.tier != null ? raw.tier : prev.tier,
      };
      result.updated++;
      dirty = true;
    }
